...
(function(f) {
  ...
  if (typeof module === 'object' && typeof module.exports === 'object') {
    module.exports = f(__webpack_require__(10),
                       __webpack_require__(4),
                       __webpack_require__(2));
  } else if ("function" === 'function' && __webpack_require__(5) != null) {
    !(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(10),
            __webpack_require__(4),
            __webpack_require__(2)], __WEBPACK_AMD_DEFINE_FACTORY__ = (f),
...
"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "c", function() { return shapes; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "b", function() { return mkMap; });
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_d3__ = __webpack_require__(20);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_d3___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_1_d3__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_sanctuary__ = __webpack_require__(1);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_sanctuary___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_2_sanctuary__);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3_libs_util__ = __webpack_require__(7);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4_libs_shapes__ = __webpack_require__(21);
...
const S = Object(__WEBPACK_IMPORTED_MODULE_2_sanctuary__["create"])({ checkTypes: false, env: __WEBPACK_IMPORTED_MODULE_2_sanctuary__["env"] });
...
const mkLegend = (svg, nodeTypeData, linkTypeData) => {
  svg.append('g').attr('transform', 'translate(20, 50)').classed('legend', true).selectAll('.legend').data(nodeTypeData).enter().append('g').attr('transform', (_, i) => 'translate(0, ' + i * 30 + ')').each(function (d) {
    ...
    __WEBPACK_IMPORTED_MODULE_1_d3__["select"](this).append('polygon').attr('points', nodeType.shape(RADIUS));
    d.label = d.label || [];
    const text = __WEBPACK_IMPORTED_MODULE_1_d3__["select"](this).append('text').attr('x', RADIUS + 5);
    d.label.forEach((t, i) => {
      text.append('tspan').attr('y', 0.5 + 1.6 * i + 'ex').classed('node', true).text(t);
    });
  });

  svg.append('g').attr('transform', 'translate(20, 180)').classed('legend', true).selectAll('.legend').data(linkTypeData).enter().append('g').attr('transform', (_, i) => 'translate(0, ' + i * 30 + ')').each(function (d) {
    __WEBPACK_IMPORTED_MODULE_1_d3__["select"](this).append('path').attr('d', linkArc({ source: { x: -RADIUS, y: RADIUS },
      target: { x: RADIUS, y: -RADIUS } })).classed(d.type, true).classed('link', true);
    d.label = d.label || [];
    const text = __WEBPACK_IMPORTED_MODULE_1_d3__["select"](this).append('text').attr('x', RADIUS + 5);
    d.label.forEach((t, i) => {
      text.append('tspan').attr('y', 0.5 + 1.6 * i + 'ex').classed('node', true).text(t);
...
const mkForce = (canvas, svg, nodeData, linkData, nodeTypeData) => {
  ...
  const force = __WEBPACK_IMPORTED_MODULE_1_d3__["layout"].force().nodes(__WEBPACK_IMPORTED_MODULE_1_d3__["values"](nodeData)).links(linkData).size([canvas.width, canvas.height]).linkDistance(({ length }) => length || defaultLength).linkStrength(({ strength }) => strength || DEFAULT_STRENGTH).gravity(0.05).charge(-300).on('tick', tick);

  const linkTypes = Object(__WEBPACK_IMPORTED_MODULE_3_libs_util__["b" /* uniquify */])(linkData.map(({ type }) => type));
  // Per-type markers, as they don't inherit styles.
  svg.append('defs').selectAll('marker').data(linkTypes).enter().append('marker').attr('id', d => d).attr('viewBox', '0 -5 10 10').attr('refX', 15).attr('refY', -1.5).attr('markerWidth', 6).attr('markerHeight', 6).attr('orient', 'auto').append('path').attr('d', 'M0,-5L10,0L0,5');

  const links = svg.append('g').selectAll('path').data(force.links()).enter().append('path').attr('class', ({ type }) => 'link ' + type).attr('marker-end', ({ type }) => 'url(#' + type + ')');

  const linkTexts = svg.append('g').selectAll('text').data(force.links()).enter().append('text').each(function (d) {
    d.label = d.label || [];
    d.label.forEach((t, i) => {
      __WEBPACK_IMPORTED_MODULE_1_d3__["select"](this).append('tspan').classed('link', true).text(t);
    });
  });

  const nodes = svg.append('g').selectAll('polygon').data(force.nodes()).enter().append('polygon').attr('class', ({ type }) => type).attr('points', d => {
    ...
    __WEBPACK_IMPORTED_MODULE_1_d3__["select"](this).classed('fixed', d.fixed = false);
  }).call(force.drag().on('dragend', function (d) {
    __WEBPACK_IMPORTED_MODULE_1_d3__["select"](this).classed('fixed', d.fixed = true);
  }));

  const nodeTexts = svg.append('g').selectAll('a').data(force.nodes()).enter().append('a').attr('xlink:href', ({ url }) => url).append('text').each(function (d) {
    d.label = d.label || [];
    d.label.forEach((t, i) => {
      __WEBPACK_IMPORTED_MODULE_1_d3__["select"](this).append('tspan').attr('x', 10).attr('y', 1.6 * i + 'ex').classed('node', true).text(t);
    });
  });
  return force;
};

const prepData = (canvas, linkData, nodeData) => {
  const xCenter = canvas.width / 2;
  const yCenter = canvas.height / 2;

  const nodeDataPrepped = S.pipe([S.keys, S.map(key => {
...
const mkMap = (canvasId, nodeData, linkData, nodeTypeData, linkTypeData) => {
  const canvas = { width: __WEBPACK_IMPORTED_MODULE_0_jquery___default()(canvasId).width(), height: __WEBPACK_IMPORTED_MODULE_0_jquery___default()(canvasId).height() };
...
const activate = map => e => {
  ...
  let first = true;
  __WEBPACK_IMPORTED_MODULE_0_jquery___default()('#underlay').addClass('inactive');
  __WEBPACK_IMPORTED_MODULE_0_jquery___default()('#overlay').removeClass('inactive');
  if (first) {
    map.start();
    __WEBPACK_IMPORTED_MODULE_0_jquery___default()('#arg-map a').click(close);
    __WEBPACK_IMPORTED_MODULE_0_jquery___default()('#arg-map > svg, #overlay').click(function (e) {
      if (e.target === this) {
        close();
      }
    });
  ...
  if (e != null) {
    ...
    const id = __WEBPACK_IMPORTED_MODULE_0_jquery___default()(e.target).attr('id');
    if (id !== undefined) {
      __WEBPACK_IMPORTED_MODULE_0_jquery___default()('g a').filter((_, a) =>
      // $FlowFixMe Doesn't know about SVG anchors
      a.href.baseVal === '#' + id).css('font-weight', 'bold');
    }
  }
};

const handler = (map, svg, canvasId) => {
  __WEBPACK_IMPORTED_MODULE_0_jquery___default()(window).resize(() => {
    svg.attr('width', __WEBPACK_IMPORTED_MODULE_0_jquery___default()(canvasId).width());
    svg.attr('height', __WEBPACK_IMPORTED_MODULE_0_jquery___default()(canvasId).height());
  });
  if (location.hash === '#arg-map') {
    activate(map)();
  }
  __WEBPACK_IMPORTED_MODULE_0_jquery___default()('a[href="#arg-map"]').click(activate(map));
};

const shapes = {
  square: __WEBPACK_IMPORTED_MODULE_4_libs_shapes__["f" /* toSvgPolygon */](__WEBPACK_IMPORTED_MODULE_4_libs_shapes__["e" /* square */]),
  circle: __WEBPACK_IMPORTED_MODULE_4_libs_shapes__["f" /* toSvgPolygon */](__WEBPACK_IMPORTED_MODULE_4_libs_shapes__["a" /* circle */]),
  diamond: __WEBPACK_IMPORTED_MODULE_4_libs_shapes__["f" /* toSvgPolygon */](__WEBPACK_IMPORTED_MODULE_4_libs_shapes__["b" /* diamond */]),
  triangle: __WEBPACK_IMPORTED_MODULE_4_libs_shapes__["f" /* toSvgPolygon */](__WEBPACK_IMPORTED_MODULE_4_libs_shapes__["g" /* triangle */]),
  hexagon: __WEBPACK_IMPORTED_MODULE_4_libs_shapes__["f" /* toSvgPolygon */](__WEBPACK_IMPORTED_MODULE_4_libs_shapes__["c" /* hexagon */]),
  pentagon: __WEBPACK_IMPORTED_MODULE_4_libs_shapes__["f" /* toSvgPolygon */](__WEBPACK_IMPORTED_MODULE_4_libs_shapes__["d" /* pentagon */])
};
...
(function(f) {
  ...
  if (typeof module === 'object' && typeof module.exports === 'object') {
    ...
    !(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(2)], __WEBPACK_AMD_DEFINE_FACTORY__ = (f),
				__WEBPACK_AMD_DEFINE_RESULT__ = (typeof __WEBPACK_AMD_DEFINE_FACTORY__ === 'function' ?
				(__WEBPACK_AMD_DEFINE_FACTORY__.apply(exports, __WEBPACK_AMD_DEFINE_ARRAY__)) : __WEBPACK_AMD_DEFINE_FACTORY__),
				__WEBPACK_AMD_DEFINE_RESULT__ !== undefined && (module.exports = __WEBPACK_AMD_DEFINE_RESULT__));
  } else {
...
"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return makeAnimationPromise; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "b", function() { return uniquify; });

/* eslint no-undef: "off" */

const makeAnimationPromise = (duration, f) => new Promise((resolve, reject) => {
  let start, prog;
  const f_ = timestamp => {
    if (start == null) {
      start = timestamp;
...
const uniquify = as => Array.from(new Set(as).values());
...
var __WEBPACK_AMD_DEFINE_FACTORY__, __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;/*              ___                 ______
               /  /\               /  ___/\
...
(function(f) {
  ...
  /* istanbul ignore else */
  if (typeof module === 'object' && typeof module.exports === 'object') {
    module.exports = f(__webpack_require__(4),
                       __webpack_require__(2));
  } else if ("function" === 'function' && __webpack_require__(5) != null) {
    !(__WEBPACK_AMD_DEFINE_ARRAY__ = [__webpack_require__(4), __webpack_require__(2)], __WEBPACK_AMD_DEFINE_FACTORY__ = (f),
				__WEBPACK_AMD_DEFINE_RESULT__ = (typeof __WEBPACK_AMD_DEFINE_FACTORY__ === 'function' ?
				(__WEBPACK_AMD_DEFINE_FACTORY__.apply(exports, __WEBPACK_AMD_DEFINE_ARRAY__)) : __WEBPACK_AMD_DEFINE_FACTORY__),
				__WEBPACK_AMD_DEFINE_RESULT__ !== undefined && (module.exports = __WEBPACK_AMD_DEFINE_RESULT__));
  } else {
...
"use strict";
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "f", function() { return toSvgPolygon; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "g", function() { return triangle; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "e", function() { return square; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "b", function() { return diamond; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "d", function() { return pentagon; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "c", function() { return hexagon; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "a", function() { return circle; });
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_sanctuary__ = __webpack_require__(1);
...
const S = Object(__WEBPACK_IMPORTED_MODULE_0_sanctuary__["create"])({ checkTypes: false, env: __WEBPACK_IMPORTED_MODULE_0_sanctuary__["env"] });

const toSvgPolygon = angles => RADIUS => S.pipe([S.map(angle => Math.cos(angle) * RADIUS + ' ' + Math.sin(angle) * RADIUS), S.joinWith(', ')])(angles);

const triangle = [3 / 6 * Math.PI, 7 / 6 * Math.PI, 11 / 6 * Math.PI];
const square = [1 / 4 * Math.PI, 3 / 4 * Math.PI, 5 / 4 * Math.PI, 7 / 4 * Math.PI];
const diamond = [0 / 2 * Math.PI, 1 / 2 * Math.PI, 2 / 2 * Math.PI, 3 / 2 * Math.PI];
const pentagon = [3 / 10 * Math.PI, 7 / 10 * Math.PI, 11 / 10 * Math.PI, 15 / 10 * Math.PI, 19 / 10 * Math.PI];
const hexagon = [1 / 6 * Math.PI, 3 / 6 * Math.PI, 5 / 6 * Math.PI, 7 / 6 * Math.PI, 9 / 6 * Math.PI, 11 / 6 * Math.PI];
