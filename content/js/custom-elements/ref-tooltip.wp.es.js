...
const stripPrefixIfPresent =
  S.curry2(
    (prefix: string, str: string): string =>
      S.fromMaybe(str)(S.stripPrefix(prefix)(str))
  )

const parseRefTitle: string => string =
  S.pipe([
    S.splitOnRegex(/\.|\?/g),
    S.at(2),
    S.fromMaybe_(() => { throw new Error('Expect ref title to be in third sentence.') }),
    S.trim,
    stripPrefixIfPresent('“')
  ])

const getRefId: HTMLElement => string =
  S.pipe([
    cite => cite.getAttribute('href'),
    S.toMaybe,
    S.fromMaybe_(() => { throw new Error('Expected citation to have link') }),
    S.stripPrefix('#'),
    S.fromMaybe_(() => { throw new Error('Expected citation link to be an anchor') })
  ])

const findRefById = (refs: Array<HTMLElement>): (string => HTMLElement) =>
  S.pipe([
    refId => S.find(ref => ref.getAttribute('id') === refId)(refs),
    S.fromMaybe_(() => { throw new Error('Expect every citation to have corresponding reference') })
  ])

const getRefTitles = (citations: Array<HTMLElement>, refs: Array<HTMLElement>): Array<string> =>
  S.map(
    S.pipe([
      getRefId,
      findRefById(refs),
